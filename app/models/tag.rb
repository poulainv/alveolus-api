...
class Tag < ActiveRecord::Base
  ...
  scope :used, lambda { joins(:tagAppRelations).joins(:webapps).where("webapps.validate = '1'").order("count(tag_app_relations.id)").group('tags.id').reverse_order.uniq }
  ## Most posted tags for one website 
  scope :most_posted, lambda { |n| joins(:tagAppRelations).joins(:webapps).where("webapps.validate = '1'").order("count(tag_app_relations.id)").group('tags.id').reverse_order.limit(n)}
