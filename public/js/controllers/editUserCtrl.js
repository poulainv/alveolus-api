...
angular.module('alveolus.editUserCtrl', []).
controller('EditUserCtrl', function($scope, $routeParams, $location, $rootScope, UserService, SessionService, CategoryService) {
	...
	if($scope.isLogged){
		UserService.editUser({userId: $scope.user.id},function(data){
			$scope.user = data
		});
	}
	else{
